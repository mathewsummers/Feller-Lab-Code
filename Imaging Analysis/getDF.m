function [stimDF,dF] = getDF(traces,stimDirs,Fs)
...
stim = 7.14;%3.6; %time of stim, sec %7.14 ; 3.48
respWindow =5;%1.5; %sec, added to stim time

[nFrames,nROIs] = size(traces);
nStims = numel(stimDirs);
nRespFrames = ceil((stim+respWindow)*Fs); %number of frames per stim plus 1.5 seconds
%nInterFrames = floor(iti*Fs); %number of frames in iti minus one
nInterFrames = round((iti+stim)*Fs - nRespFrames);
