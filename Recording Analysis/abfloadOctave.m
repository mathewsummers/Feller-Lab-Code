function [d,si]=abfloadOctave(fn,varargin)
...
% 2016-05-20 Mathew made edits to abfload to make it compatible with
% Octave. Matlab can fread single bits, whereas Octave cannot, so added an
% if statement in the ReadSection subroutine to handle this.
...
switch fFileSignature
 case 'ABF2'
  ...
  headPar={
     ...
     'fFileVersionNumber',4,'int',[-1 -1 -1 -1];
...
for i=1:length(s)
  if strcmpi(s(i).numType,'bit1')
    pos = ftell(fid);
    eval(['[Section.' s(i).name ',n]=fread(fid,' num2str(s(i).number) ',''' 'uint8' ''');']);
    fseek(fid,pos+1,'bof');
  else
